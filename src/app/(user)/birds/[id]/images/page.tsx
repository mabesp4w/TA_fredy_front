...
export default function BirdImagesPage() {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-base-200 rounded-lg p-6 mb-6">
        <h1 className="text-2xl font-bold text-primary mb-2">
          {currentBird.bird_nm} Images
        </h1>
        <p className="text-lg italic text-gray-600 mb-2">
          {currentBird.scientific_nm}
        </p>
        {currentBird.description && (
          <p className="text-gray-600">{currentBird.description}</p>
        )}
      </div>

      {/* Images List */}
      <ImageList birdId={birdId} showBirdFilter={false} />
    </div>
  );
}
